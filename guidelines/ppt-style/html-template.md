...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ===========================================
           ANIMATIONS
           Core rule: content stays visible without external state.
           =========================================== */
        /* Pattern A (recommended): self-start keyframes */
        .reveal {
            opacity: 1;
            transform: none;
            animation: fade-up var(--duration-normal) var(--ease-out-expo) both;
        }
        @keyframes fade-up {
            from {
                opacity: 0;
        ...
        /* Pattern B (optional): state-driven, but fallback remains visible */
        .reveal-state {
            opacity: 1;
            transform: translateY(0);
...
2. **Intersection Observer** — Optional enhancement for state-driven motion:
   - May add `.visible` class when slides enter viewport
   - Never make `.visible` the only way core text becomes visible
   - If observer fails, title/subtitle/body content must remain readable
